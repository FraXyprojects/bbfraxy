...
function processNextGroup(state,mount,renderFinal){
  ...
  if(!group){renderFinal(state.placed);return;}
  if(group.length===1){state.placed.push(group[0]);state.groupIndex+=1;processNextGroup(state,mount,renderFinal);return;}
...
function renderWheel(state,mount,renderFinal){
  ...
  const labels=group.map((player,i)=>{
    ...
    return `<span class="wheel-label" style="left:${x}%;top:${y}%;--label-color:${player.color}" title="${esc(player.name)}">${esc(player.name)}</span>`;
  }).join('');
  mount.innerHTML=`<div class="tiebreak-card"><div class="eyebrow">FRAXY // DECISION ROUND</div><h3>Kolo rozhodnutí</h3><p>Skóre mezi <strong>${group.map(p=>`<span style="color:${p.color}">${esc(p.name)}</span>`).join(', ')}</strong> je nerozhodné. O vítězi rozhodne náhoda.</p><div class="wheel-stage"><div class="wheel-pointer" aria-hidden="true"></div><div class="wheel" id="wheel" style="--wheel-gradient:conic-gradient(${gradient})">${labels}<div class="wheel-center">?</div></div></div><div id="wheel-result" class="wheel-result" aria-live="polite"></div><button class="btn" id="spin-wheel">🎡 Zatočit kolem</button></div>`;
  mount.querySelector('#spin-wheel').onclick=()=>spin(state,mount,renderFinal);
}

function randomIndex(max){
  if(globalThis.crypto?.getRandomValues){
    const arr=new Uint32Array(1);globalThis.crypto.getRandomValues(arr);return Math.floor((arr[0]/4294967296)*max);
  }
...
function spin(state,mount,renderFinal){
  ...
  window.setTimeout(()=>{
    ...
    window.setTimeout(()=>{
      if(state.currentGroup.length>1){renderWheel(state,mount,renderFinal);return;}
